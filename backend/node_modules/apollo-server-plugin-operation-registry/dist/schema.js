...
async function generateSchemaHash(schema) {
    ...
    if (!result || !result.data || !result.data.__schema) {
        throw new Error(`${common_1.pluginName}: Unable to generate server introspection document.`);
    }
    ...
    return crypto_1.createHash('sha512').update(stringifiedSchema).digest('hex');
}
